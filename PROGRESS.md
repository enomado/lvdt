...
- [x] **Stage 6 — детектор качества + AGC через встроенный PGA OPAMP1/2 — host‑тесты зелёные, ждёт bring‑up на железе.** Три **разных** ошибки тракта различаются и видны: символ напротив `A`/`B` в defmt и на OLED — `S`aturated > `L`ow > `H`armonic > `.` ok.
  ...
  - **AGC через OPAMPx PGA.** OPAMP1 (PA3 → ADC1_IN13) и OPAMP2 (PB14 → ADC2_IN16) подняты в PGA‑mode (`VM_SEL=Pga`, `VP_SEL=Vinp1`, `OPAINTOEN=Adcchannel`), стартовый gain ×2. Раз в окно усреднения (`SMOOTHING_BLOCKS=64` блоков ≈ 25.6 мс) `agc::tick`: решение считается по каждому каналу, но применяется общая ступень A/B. Если любой канал клиппит или горячий (`mag>75%`) — оба шагают вниз; вверх оба идут только когда оба слабые (`mag<25%`); иначе hold. Между шагами lockout 1 окно. Шкала ×2/×4/×8/×16/×32/×64 — 6 ступеней, всего ×32 динамики; окно 25–75% (3×) шире фактора смены 2×. `decide` и `tick` host‑тестируются через mock PGA.
...
- [x] **USER‑кнопка с «фонарными» паттернами — host‑тесты зелёные, ждёт bring‑up на железе.** Одна кнопка PC13 (active high, `into_pull_down_input`), polling 8 мс из async RTIC task. `button::ButtonFsm` распознаёт четыре паттерна:
  ...
  - `L` (long, ≥500 ms) → `Position` — большой `POS +12.34%` сверху, `A.x4 B.x4` снизу. Числитель `(B−A)/(B+A)` нормирован на gain каждого канала как страховка от переходных окон, хотя AGC теперь держит A/B на общей ступени. Если любой канал флагнут `S`, `L` или `H`, позиция скрывается как `POS    --`, а нижняя строка оставляет qualifier+gain.
  - `LSS` (long+short+short) → `Raw` — `AS000 ~99.9% / BS001 ~80.5%` (sat_count + sine purity = fund/total energy). 100 % — чистый синус, ≈81 % — square wave, ниже — DC bias / асимметричный clip / обрыв. При mag <1 % REFERENCE — `~  --%`, чтобы шум ADC не маскировался под «грязный» сигнал. Удобно при bring‑up.
...
## Улучшения
- [x] **DMA buffer safety.** ADC DMA пишет в двойной circular buffer `2×LUT_LEN`; `HTIF` отдаёт первую стабильную половину, `TCIF` — вторую. `snapshot(block)` больше не копирует live‑половину, в которую прямо сейчас пишет DMA.
- [x] **Host‑testable AGC tick + связка A/B.** `agc::tick` вынесен из ARM‑only кода, тестируется через mock PGA и держит A/B на общей ступени: down при hot/clipping любого канала, up только когда оба weak.
...
- **A vs B при разных gain.** AGC теперь принудительно сводит A/B к общей ступени, но экран всё равно считает позицию через `mag/gain` как страховку от переходных окон и старого состояния после reset/экспериментов. На железе проверить, что при сменах gain позиция не прыгает.
